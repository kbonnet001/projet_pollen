...
from ik_methods import reduce_model_pin_arm, get_joints_from_chosen_method, get_current_joints
...
def go_to_pose_pink(model, data, reachy: ReachySDK, pose: npt.NDArray[np.float64], arm: str) -> None:
    if arm == "r_arm":
        #ik = reachy.r_arm.inverse_kinematics(pose)
        ik = get_joints_from_chosen_method(model, data, pose, "r", "pink")
        # real_pose = reachy.r_arm.forward_kinematics(ik)
        # pose_diff = np.linalg.norm(pose - real_pose)
        # print(f"pose diff {pose_diff}")
        for joint, goal_pos in zip(reachy.r_arm.joints.values(), ik):
        ...
        ik = get_joints_from_chosen_method(model, data, pose, "l", "pink")
        # real_pose = reachy.l_arm.forward_kinematics(ik)
        # pose_diff = np.linalg.norm(pose - real_pose)
        # print(f"pose diff {pose_diff}")
        for joint, goal_pos in zip(reachy.l_arm.joints.values(), ik):
...
prefix : str
    ...
    new_joints = get_current_joints(prefix)
    j=0
    for i in indices:
    ...
    go_to_pose_pink(model, data, reachy, np.array(pose), arm)
    time.sleep(0.001)

def make_line(
    models, datas,
    reachy: ReachySDK, start_pose: npt.NDArray[np.float64], end_pose: npt.NDArray[np.float64], nbr_points: int = 300
) -> None:
    ...
    for i in range(nbr_points):
        ...
        go_to_pose_pink(models[0], datas[0], reachy, pose, "r_arm")
        ...
        #go_to_pose_pink(models[1], datas[1], reachy, l_pose, "l_arm")
...
def make_rectangle(
    ...
    for i in range(number_of_turns):
        make_line(models, datas, reachy, np.array([A, orientation]), np.array([B, orientation]), nbr_points)
        make_line(models, datas, reachy, np.array([B, orientation]), np.array([C, orientation]), nbr_points)
        make_line(models, datas, reachy, np.array([C, orientation]), np.array([D, orientation]), nbr_points)
        make_line(models, datas, reachy, np.array([D, orientation]), np.array([A, orientation]), nbr_points)
...
def main_test() -> None:
    ...
    model_r, data_r = reduce_model_pin_arm("r")
    model_l, data_l = reduce_model_pin_arm("l")
    models = [model_r, model_l]
    datas = [data_r, data_l]

    print("Set to Zero pose ...")
    move_ids = reachy.set_pose("default")
    while not reachy.is_move_finished(move_ids.r_arm):
        time.sleep(0.1)

    while not reachy.is_move_finished(move_ids.l_arm):
        time.sleep(0.1)
...
##################################################################
    ...
    for i in range(10000):
        moving_q =  max * np.sin(-step*i)
        move_q(models[0], datas[0], reachy, np.array([1]), np.array([moving_q]), "r")
        print(f"q_objectif = {int(moving_q)}")
    ...
    radius = 0.5
    make_circle(models, datas, reachy, center, orientation, radius, 1000, 3)
    ...
    min_radius = 0.1
    max_radius = 1
    #make_spiral(models, datas, reachy, center, orientation, min_radius, max_radius, number_of_turns=5)
    ...
    #make_rectangle(models, datas, reachy, A, B, C, D)
