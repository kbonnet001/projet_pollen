# /home/reachy/dev/reachy2-sdk/src/examples/draw_square_method.py
...
# from ik_pink import reduce_model_pin_arm, get_joints_from_pink
# from ik_pinocchio import get_joints_from_pin
from ik_methods import get_joints_from_chosen_method, reduce_model_pin_arm
...
def draw_square_with_pin(model, data, reachy: ReachySDK, method, step=0.05) -> None:
    ...
    for z in np.arange(-0.2+step, 0.0+step, step):
        ...
        joints_deg = get_joints_from_chosen_method(model, data, target_pose, "r", method)
        move_id = reachy.r_arm.goto_joints(joints_deg, 0.5, degrees=True)

    # wait for movement to finish
    while not reachy.is_move_finished(move_id):
        time.sleep(0.1)
    current_pos = reachy.r_arm.forward_kinematics()
    input(f"Pose B (with {method}) :\ncurrent pose = {current_pos}\n----")
...
if __name__ == "__main__":
    ...
    method = "pinocchio" #pin/ pinocchio or pink
    draw_square_with_pin(model, data, reachy, method)
